    ...
    for(i=1; i<=nf; i++)
    {
        for(j=1; j<=nb; j++)
        {
            ...
            if(bf[j]!=1)
            {
                ...
                if(temp >= 0)         // block size file এর থেকে বড় বা সমান হতে হবে
                    if(highest < temp) // যদি এখন পর্যন্ত যেই ফাঁকা জায়গা পাওয়া গেছে তার থেকে বেশি হয়
                    {
                        ff[i] = j;    // এই block টি file এর জন্য নির্বাচন করা হলো
                        highest = temp; // সবচেয়ে বড় ফাঁকা জায়গা আপডেট করলাম
                    }
            }
        }
        frag[i] = highest;  // ফাঁকা জায়গা (fragmentation) হিসেবে রাখলাম
        bf[ff[i]] = 1;      // সেই block টি allocate হয়ে গেছে হিসেবে চিহ্নিত করলাম
        highest = 0;        // পরের file এর জন্য highest শূন্য করে দিলাম
    }
