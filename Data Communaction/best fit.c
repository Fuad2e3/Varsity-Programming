...
void main()
{
    ...
    for(i=1; i<=nf; i++)
    {
        int small = 100000;  // একটা বড় মান দিয়ে শুরু করছি
        for(j=1; j<=nb; j++)
        {
            ...
            if(bf[j]!=1)
            {
                ...
                if(temp >= 0)         // block size file এর থেকে বড় বা সমান হতে হবে
                    if(temp < small) // যদি এখন পর্যন্ত যেই ফাঁকা জায়গা পাওয়া গেছে তার থেকে বেশি হয়
                    {
                        ff[i] = j;    // এই block টি file এর জন্য নির্বাচন করা হলো
                        small = temp; // সবচেয়ে বড় ফাঁকা জায়গা আপডেট করলাম
                    }
            }
        }
        frag[i] = small;  // ফাঁকা জায়গা (fragmentation) হিসেবে রাখলাম
